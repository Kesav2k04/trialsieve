...
| step                                               | model calls | from cassette | prompt tokens | completion tokens | wall clock     | at an illustrative rate |
|----------------------------------------------------|-------------|---------------|---------------|-------------------|----------------|-------------------------|
...
| critic probe, planted defects                      | 25          | 25            | 19,497        | 3,710             | 0 s            | $0.02                   |
...
| **total**                                          | **1,291**   |               | **2,627,232** | **174,395**       | **2 h 23 min** | **$1.22**               |

The right-hand column is an estimate at an illustrative mid-tier hosted rate of $0.30 in / $2.50 out per million tokens, not a quote from a named vendor. Each row is rounded to the cent, so adding the column comes to a cent or two more than the total, which is computed once from the raw token counts rather than by summing rounded rows. It is not what this
