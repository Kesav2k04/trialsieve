...
{
 "category": "lab_value",
 "criterion_id": "NCT06983054-INC-04-CF",
 "kind": "inclusion",
 "source_text": "Overweight or obese with BMI: >34.2 kg/m2"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `af18a84d18206a1e`
...
Criterion (inclusion, category lab_value):
  Overweight or obese with BMI: >34.2 kg/m2
...
### 4. llm_response (cassette, 93 tok, 65.386s)

```
```json
{
  "checkable": true,
  "reason": "BMI is a standard numeric measurement recorded in structured vital signs that can be evaluated against the threshold of >34.2 kg/m2 as of the index date.",
  "concepts": [
    {
      "name": "Body mass index",
      "domain": "measurement",
      "intent": "most recent numeric BMI value exceeding 34.2 kg/m2"
    }
  ]
}
```
...
### 9. llm_response (cassette, 90 tok, 85.431s)
...
```json
{
  ...
  "note": "Standard clinical EHR observation names, abbreviations, and pediatric index variants for body mass index."
}
```
```
...
### 12. llm_request -> gemini-3.7-flash-medium  cassette `b2ac27c43ea7aabf`
...
Concept: Body mass index
Record domain: observation
Meaning required: most recent numeric BMI value exceeding 34.2 kg/m2
...
### 13. llm_response (cassette, 318 tok, 68.224s)
...
```json
{
  ...
  "rejected": [
    {
      "code": "59576-9",
      "why": "BMI percentile per age and gender rather than numeric BMI value (kg/m2)"
    },
    {
      "code": "29463-7",
    ...
    {
      "code": "8310-5",
      "why": "body temperature measurement, not body mass index"
    },
    {
      "code": "2713-6",
      "why": "oxygen saturation measurement, unrelated to BMI"
    },
    {
      "code": "14959-1",
      "why": "microalbumin to creatinine ratio, unrelated to BMI"
    },
    {
      "code": "2708-6",
      "why": "arterial blood oxygen saturation, unrelated to BMI"
    },
    {
      "code": "2502-3",
      "why": "iron saturation in serum/plasma, unrelated to BMI"
    },
    {
      "code": "788-0",
      "why": "erythrocyte distribution width (RDW), unrelated to BMI"
    },
    {
      "code": "59408-5",
      "why": "pulse oximetry oxygen saturation, unrelated to BMI"
    },
    {
      "code": "33914-3",
      "why": "estimated glomerular filtration rate (eGFR), unrelated to BMI"
    }
  ],
  "confidence": "high"
}
...
### 15. llm_request -> gemini-3.7-flash-medium  cassette `bce8b4a57561c8ae`
...
### 16. llm_response (cassette, 105 tok, 121.073s)
...
```json
{
  ...
  "unit_note": "kg/m2",
  "absence_note": "No query object requiring absent_means was used; BMI is evaluated as a derived clinical observation.",
  "confidence": "high"
}
```
```
