...
```json
{
 ...
 "criterion_id": "NCT06983054-INC-05",
 "kind": "inclusion",
 "source_text": "Both sexes (females must be post-menopausal; no menses >1 year; in case of doubt, Follicle-Stimulating Hormone (FSH) will be determined with cut-off defined as >31 U/L)"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `82d953345e28168f`
...
### 4. llm_response (openai, 138 tok, 62.73031711578369s)

```
```json
{
  "checkable": false,
  "reason": "Menstrual history (no menses for >1 year) is not tracked in structured coded medical records, and resolving postmenopausal status in doubtful cases relies on prospective protocol testing and clinical judgment.",
  "concepts": [
    {
      "name": "Postmenopausal state",
      "domain": "condition",
      "intent": "coded diagnosis of postmenopausal status"
    },
    {
      "name": "Follicle-Stimulating Hormone",
      "domain": "measurement",
      "intent": "serum FSH level >31 U/L"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "Menstrual history (no menses for >1 year) is not tracked in structured coded medical records, and resolving postmenopausal status in doubtful cases relies on prospective protocol testing and clinical judgment."
}
```
