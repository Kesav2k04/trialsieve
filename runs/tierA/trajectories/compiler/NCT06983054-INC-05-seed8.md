...
### 3. llm_request -> gemini-3.7-flash-medium  cassette `9aebe69435ddf30f`
...
### 4. llm_response (cassette, 137 tok, 31.815s)

```
```json
{
  "checkable": false,
  "reason": "Menstrual history ('no menses >1 year') is not available in structured coded data, and conditional testing ('in case of doubt') requires prospective clinical evaluation.",
  "concepts": [
    {
      "name": "Postmenopausal state",
      "domain": "condition",
      "intent": "documented postmenopausal diagnosis"
    },
    {
      "name": "Follicle-stimulating hormone",
      "domain": "measurement",
      "intent": "laboratory measurement of FSH to confirm post-menopausal status"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "Menstrual history ('no menses >1 year') is not available in structured coded data, and conditional testing ('in case of doubt') requires prospective clinical evaluation."
}
```
