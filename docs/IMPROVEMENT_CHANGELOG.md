...
## 58. The strongest reason to discount every accuracy number was not stated

**Found by** a reviewer asking who wrote the labels. `docs/EVAL_PROTOCOL.md`
section 6 describes two labelling routes and calls the first "hand-authored gold
predicates". It never says by whom.

**What was wrong.** By the author of the system being scored. Every accuracy figure
in this repository is agreement with labels one of whose two routes was written by
the person who wrote the compiler, and a reader had to infer that from the absence
of any other name. Nothing was hidden and nothing was disclosed either, which is
the worse of the two, because a reader who works it out for themselves has to
wonder what else they are expected to work out.

The bias runs one way. A gold predicate written by the same mind that wrote the
compiler can encode the same reading of a criterion, and then agreement measures
one mind against itself rather than against the protocol.

**What changed.** It is stated, in the three places a reader meets the numbers:
`results/RESULTS.md` opens the label noise floor with it, `README.md` says it beside
the headline pair, and `SUBMISSION.md` carries a section named for the question.
Each says the same three things: who wrote gold, which direction that biases, and
what bounds it. What bounds it is real and was already in the repository, so this
entry adds no machinery: Checker B works from the criterion prose alone on a
different model family with no sight of the compiled predicate, its blindness is a
fact about commit order rather than a claim, and the 23.3% at which the two
labellers disagree is published as a floor that marks two comparisons TrialSieve
loses as uninterpretable.

What none of it buys is clinical ground truth, and the report now says so in the
sentence a reader reaches before the agreement figures rather than after them.

| | before | now |
|---|---|---|
| places naming who authored gold | 0 | **3** |
| places stating which way that biases the result | 0 | **3** |
| what the accuracy figures are described as agreement with | gold | **two readings of the protocol text, one of them the author's** |

**Evidence.** `results/RESULTS.md`, the paragraph headed *Who wrote gold*, which is
generated by `scripts/report.py` rather than typed into the document, so it cannot
be edited out of the report without editing the generator.

## 59. A co-primary outcome was computed from the first run and reported nowhere

**Found by** a reviewer reading `docs/EVAL_PROTOCOL.md` for outcomes and then
searching every document for each one by name.

**What was wrong.** Section 3 registers `resolved_correct_per_screen` as a
co-primary, and says what it is for in the same breath: *so an arm cannot win by
abstaining*. `evaluation/score.py` has computed it for every arm since the first
scored run. It appeared **zero times** in `README.md`, `SUBMISSION.md`,
`docs/SCORECARD.md`, `results/RESULTS.md`, or anywhere in this changelog.

On the paired cells B2 resolves **3.23** correctly per screen and TrialSieve
resolves **2.77**. The baseline wins it by 17%.

So the single guard registered against this design's central objection was
computed, and the page that explains the low coverage row as lower on purpose did
not carry it. Nobody removed it. It was written into `results.json` and never
carried forward, which is the version of this that is hardest to catch, because
there is no deletion to find and no sentence that is false. The reader is simply
not told.

**What changed.** It is a column in both arm tables in `results/RESULTS.md`, and
the section names the winning arm in bold under the table whichever arm that is. It
is a row in `docs/SCORECARD.md`, and the paragraph that used to open *Reading the
coverage row honestly* now opens with the row this project loses instead. It is a
bullet in `SUBMISSION.md`, beside the fact that the primary outcome is VOID for
both arms, which that page also did not say.

`tests/test_registered_outcomes_are_reported.py` parses the co-primary's name out
of the protocol and requires it to appear in both reporting documents, requires the
report to name the arm that wins it, and fails first if the protocol ever stops
registering one, because a check over an empty set of names reports a pass it did
not earn.

| | before | now |
|---|---|---|
| documents naming the registered co-primary | 0 | **4** |
| documents saying which arm wins it | 0 | **3** |
| gates over the reporting of a registered outcome | 0 | **4** |

**Evidence.** `python -m pytest tests/test_registered_outcomes_are_reported.py -q`.
Delete the column from `scripts/report.py` and it names the document the outcome
went missing from.

## 60. Four of five registered predictions were never compared to an outcome

**Found by** the same reading. Prediction 1, coverage, is scored in the README.
Predictions 2, 3, 4 and 5 were registered before the first scored run and then
never mentioned again.

**What was wrong.** A pre-registration is worth exactly the branch nobody wanted.
Registering five and reporting the one that was closest to holding turns it into a
gesture, and this repository leans on it hard: the protocol is committed before the
first result and that commit order is itself a published claim.

Entry 57 is what prediction 4 cost. The other three had smaller answers and one of
them had none at all: prediction 2 says B2 separates from TS on *degraded records*,
and B2 only ever ran at k = 0, so that half has no data behind it in either
direction. That is not a prediction that failed. It is a prediction the run was
never built to test, and reporting it as untested is the only honest option.

**What changed.** `scripts/report.py` scores all five into a generated table in
`results/RESULTS.md`, with the registered wording, the measured outcome and a
verdict per row. Where the outcome is a number this report already computes, the
verdict is computed with it. Where it is not, the row says *partly untested* rather
than guessing.

**2 of 5 held as registered**, and the table says so in bold under itself. Coverage
came in below its band, k = 0 was wrong in its premise, and one is untestable with
what ran. None of that is a defect in the result. Leaving it unreported was a
defect in the reporting, and it is the defect this project would have been least
able to answer for, because the pre-registration is the reason to believe anything
else in here.

| | before | now |
|---|---|---|
| registered predictions compared to an outcome | 1 of 5 | **5 of 5** |
| of those, held as registered | not stated | **2, stated in the table** |
| predictions the run could not test | not stated | **1, named as untestable** |

**Evidence.** `python -m pytest tests/test_registered_outcomes_are_reported.py -q`
requires five scored rows, each with a measurement and a verdict, and requires the
table to be in the report. Drop a row from the generator and it says which.

## 61. The probe's weakest class was reported at the better of two rates

**Found by** a reviewer counting files. `docs/CRITIC_PROBE.md` reported the
`absence` class at **3 of 4**. `runs/tierA/trajectories/critic_probe/` ships five
absence probes, and one of the extra ones is a miss, so counting every probe in the
tree gives **3 of 5**.

**What was wrong.** Not a chosen denominator, which is what it looked like. The
document scores one probe run of six predicates and reproduces from cassettes
exactly. An earlier and wider probe ran before it, its logs were kept, and its
numbers were never published. The two disagree and the document gave a reader no
way to tell which they were holding.

The effect is the same either way, and that is the part worth keeping: the better
of two available rates was the one on the page, in the one class this critic is
weak in, in the class that produced 358 of 424 wrong exclusions in the run the
probe was written against. Intent does not enter into it. A reader counting the
directory computes 3 of 5 and finds a document saying 3 of 4.

**What changed.** `evaluation/critic_probe.py` now counts the trajectory directory
itself and prints that tally **first**, above the scored run, with the sentence
naming `absence` as 3 of 5 and saying plainly that the wider figure is the one to
hold the component to. The scored run stays because it is the one that regenerates
from cassettes. The extra predicates are named.

`tests/test_critic_probe_matches_the_trajectories.py` recounts every probe
trajectory and compares it to the published wider table, checks that every scored
predicate has its log in the tree, and requires the worse absence rate to appear
beside the better one. That third check exists because the wider table's only
purpose is to be the less flattering of the two, which makes it the thing most
likely to disappear the next time the generator is edited.

| | before | now |
|---|---|---|
| tallies published | 1, the smaller run | **2, wider one first** |
| the `absence` rate a reader computes from the tree | 3 of 5, unstated | **3 of 5, stated** |
| gates reconciling the document with the trajectories | 0 | **3** |
