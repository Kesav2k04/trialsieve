### 1. instructions (prompt compiler-v3)

```
You prepare clinical trial eligibility criteria for automated checking against
structured medical records.

You are careful about what a medical record can and cannot settle. Most criteria
in a real protocol cannot be settled from a record at all, and saying so is the
correct answer rather than a failure.

Decide whether this criterion can be checked from a structured medical record,
and list the clinical concepts it needs.

Criterion ({kind}, category {category}):
  {text}

A criterion is checkable ONLY if every part of it can be settled from coded
record data: demographics, coded diagnoses, coded medications, coded procedures,
or numeric laboratory and vital-sign values, with dates.

The record carries an index date: the day this prescreen is being run. Study
events that have not happened yet are anchored to it. "at screening", "at
randomisation", "at the time of signing the informed consent form", "at baseline",
"prior to randomisation" all mean "as of the index date", and a window measured
back from one of them is measured back from the index date. That substitution is
what prescreening is, so it does not make a criterion uncheckable. Say in `reason`
that you used it, so the reviewer sees the assumption.

What stays uncheckable is the participant, not the calendar: whether they will
consent, whether they are able to comply, what the investigator will judge.

It is NOT checkable if it needs any of:
  - consent, willingness, capacity, ability to comply or to attend
  - the investigator's opinion, or a judgement about suitability
  - a measurement a record does not normally hold (waist circumference,
    performance status, imaging read-outs, questionnaire scores, elastography)
  - counting prior lines of therapy, or a washout expressed in half-lives
  - a bound relative to a laboratory's own reference range, such as "2.5x the
    upper limit of normal", since the limit is not a fixed number
  - contraception, pregnancy intention, or residence and travel plans
  - enrolment in another study

Return JSON only:

{{"checkable": true,
  "reason": "one sentence; if not checkable, name the specific blocker",
  "concepts": [{{"name":"SGLT2 inhibitor","domain":"medication",
                 "intent":"currently prescribed drug of this class"}}]}}

`concepts` lists every clinical thing that must be looked up by code. Leave it
empty when the criterion uses only age or sex. Use the exact domain the record
would store it in.

---

You translate one clinical trial eligibility criterion into a formal predicate.

You are precise and literal. You express exactly what the criterion says, and you
use only the codes you are given.

Translate this criterion into a predicate.

Criterion ({kind}, category {category}):
  {text}

PREDICATE GRAMMAR

expr:
  {"op":"and","args":[expr,...]}
  {"op":"or","args":[expr,...]}
  {"op":"not","arg":expr}
  {"op":"at_least","n":INT,"args":[expr,...]}        for "at least N of the following"
  {"op":"compare","cmp":">|>=|<|<=|==|!=","left":value,"right":value}
  {"op":"between","value":value,"low":NUM,"high":NUM,"inclusive":[BOOL,BOOL]}
  {"op":"exists","query":query}
  {"op":"const","value":"TRUE|FALSE|UNKNOWN"}

value:
  {"val":"age"}                                       age in years at the screening date
  {"val":"sex"}
  {"val":"literal","number":NUM,"unit":"STR"}
  {"val":"observation","codes":["LOINC",...],"unit":"STR","agg":"latest|min|max|first",
   "within_days":INT|null}
  {"val":"derived","name":"egfr_ckdepi_2021|bmi|systolic_bp|diastolic_bp","within_days":INT|null}
  {"val":"count","query":query}

query:
  {"domain":"condition|medication|procedure|observation","codes":["CODE",...],
   "broader_codes":["CODE",...],
   "within_days":INT|null,"active_only":BOOL,"absent_means":"false|unknown"}

RULES

* `unit` on an observation is the unit the CRITERION is written in, not the unit
  the record stores. Conversion is handled for you. State the criterion's unit.
* `within_days` encodes a temporal window. "within the past 3 months" is 91.
  "within 6 months" is 183. "within 1 month" is 30. null means no window.
* `absent_means` decides what an empty result means, and it is the most
  consequential field here.
    "false"   - this record can be trusted to be complete for these codes, so
                nothing found means the patient does not have it. Reasonable for
                a reconciled medication list or a coded problem list.
    "unknown" - silence proves nothing. Correct for anything that may have
                happened at another site, for events like a hospitalisation, and
                for anything acute or recent.
  When in doubt choose "unknown". A wrong "false" rules a patient out of a trial
  on the strength of a gap in their record, and nobody ever audits that.
* `broader_codes` are codes this site uses that CONTAIN the concept without
  establishing it, listed for you in the GROUNDED CONCEPTS block. Copy them into
  the query when they are given. They behave differently from `codes` on purpose:
  a broader code found in the record makes the answer undetermined rather than
  true, while finding nothing at all still means whatever `absent_means` says.
  That is not a hedge, it is the actual state of knowledge. A site that records
  one unspecified diabetes code cannot tell you a patient has type 2, and can
  still tell you a patient has no diabetes of any kind.
* Never invent a code. Use only codes from the GROUNDED CONCEPTS block.
* An exclusion criterion is expressed as the thing that would EXCLUDE the
  patient. `exists(current SGLT2 inhibitor)` is a correct exclusion predicate.
* Model the criterion as written. Do not add a clinical safety margin, do not
  widen a range, and do not add a condition the text does not state.

GROUNDED CONCEPTS
=================
{grounded}

WORKED EXAMPLES

Criterion (inclusion): "HbA1c 6.5-10%"
  Grounded: HbA1c (observation) codes ["4548-4"]
  {"expr": {"op":"between",
            "value":{"val":"observation","codes":["4548-4"],"unit":"%",
                     "agg":"latest","within_days":null},
            "low":6.5,"high":10.0,"inclusive":[true,true]}}

Criterion (inclusion): "Age 18 - 85 years of age"
  Grounded: none
  {"expr": {"op":"between","value":{"val":"age"},"low":18,"high":85,
            "inclusive":[true,true]}}

Criterion (inclusion): "BMI greater than or equal to 27.0 kg/m^2"
  Grounded: Body Mass Index (observation) codes ["39156-5"]
  {"expr": {"op":"compare","cmp":">=",
            "left":{"val":"derived","name":"bmi","within_days":null},
            "right":{"val":"literal","number":27.0,"unit":"kg/m2"}}}

Criterion (exclusion): "Myocardial infarction or stroke within 180 days before screening"
  Grounded: myocardial infarction (condition) ["22298006","399211009"],
            stroke (condition) ["230690007"]
  {"expr": {"op":"or","args":[
      {"op":"exists","query":{"domain":"condition","codes":["22298006","399211009"],
                              "within_days":180,"absent_means":"unknown"}},
      {"op":"exists","query":{"domain":"condition","codes":["230690007"],
                              "within_days":180,"absent_means":"unknown"}}]}}
  absent_means is "unknown" because a myocardial infarction may have been treated
  at another hospital, and silence must not clear the patient.

Criterion (inclusion): "at least one of: blood pressure >=130 systolic, or
                        triglycerides >=150 mg/dL"
  {"expr": {"op":"at_least","n":1,"args":[
      {"op":"compare","cmp":">=","left":{"val":"derived","name":"systolic_bp",
                                          "within_days":null},
       "right":{"val":"literal","number":130,"unit":"mmHg"}},
      {"op":"compare","cmp":">=","left":{"val":"observation","codes":["2571-8"],
                                          "unit":"mg/dL","agg":"latest","within_days":null},
       "right":{"val":"literal","number":150,"unit":"mg/dL"}}]}}

Return JSON only:

{{"expr": <expr>,
  "unit_note": "which units the criterion is written in",
  "absence_note": "why each absent_means was chosen",
  "confidence": "high|medium|low"}}
```

### 2. input

```json
{
 "category": "temporal_event",
 "criterion_id": "NCT06717698-EXC-01",
 "kind": "exclusion",
 "source_text": "Myocardial infarction, stroke, transient ischaemic attack, or hospitalization for unstable angina pectoris within 180 days before screening."
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `d86a56385c2f6b3d`

```
[system]
You prepare clinical trial eligibility criteria for automated checking against
structured medical records.

You are careful about what a medical record can and cannot settle. Most criteria
in a real protocol cannot be settled from a record at all, and saying so is the
correct answer rather than a failure.

[user]
Decide whether this criterion can be checked from a structured medical record,
and list the clinical concepts it needs.

Criterion (exclusion, category temporal_event):
  Myocardial infarction, stroke, transient ischaemic attack, or hospitalization for unstable angina pectoris within 180 days before screening.

A criterion is checkable ONLY if every part of it can be settled from coded
record data: demographics, coded diagnoses, coded medications, coded procedures,
or numeric laboratory and vital-sign values, with dates.

The record carries an index date: the day this prescreen is being run. Study
events that have not happened yet are anchored to it. "at screening", "at
randomisation", "at the time of signing the informed consent form", "at baseline",
"prior to randomisation" all mean "as of the index date", and a window measured
back from one of them is measured back from the index date. That substitution is
what prescreening is, so it does not make a criterion uncheckable. Say in `reason`
that you used it, so the reviewer sees the assumption.

What stays uncheckable is the participant, not the calendar: whether they will
consent, whether they are able to comply, what the investigator will judge.

It is NOT checkable if it needs any of:
  - consent, willingness, capacity, ability to comply or to attend
  - the investigator's opinion, or a judgement about suitability
  - a measurement a record does not normally hold (waist circumference,
    performance status, imaging read-outs, questionnaire scores, elastography)
  - counting prior lines of therapy, or a washout expressed in half-lives
  - a bound relative to a laboratory's own reference range, such as "2.5x the
    upper limit of normal", since the limit is not a fixed number
  - contraception, pregnancy intention, or residence and travel plans
  - enrolment in another study

Return JSON only:

{"checkable": true,
  "reason": "one sentence; if not checkable, name the specific blocker",
  "concepts": [{"name":"SGLT2 inhibitor","domain":"medication",
                 "intent":"currently prescribed drug of this class"}]}

`concepts` lists every clinical thing that must be looked up by code. Leave it
empty when the criterion uses only age or sex. Use the exact domain the record
would store it in.
```

### 4. llm_response (openai, 298 tok, 19.30505394935608s)

```
```json
{
  "checkable": true,
  "reason": "All specified cardiovascular and cerebrovascular events and hospitalizations can be identified via coded diagnoses and inpatient encounter records within the 180 days prior to the index date, anchoring screening to the index date.",
  "concepts": [
    {
      "name": "Myocardial infarction",
      "domain": "condition",
      "intent": "diagnosis of myocardial infarction within 180 days prior to index date"
    },
    {
      "name": "Stroke",
      "domain": "condition",
      "intent": "diagnosis of ischemic or hemorrhagic stroke within 180 days prior to index date"
    },
    {
      "name": "Transient ischaemic attack",
      "domain": "condition",
      "intent": "diagnosis of transient ischemic attack within 180 days prior to index date"
    },
    {
      "name": "Unstable angina pectoris",
      "domain": "condition",
      "intent": "diagnosis of unstable angina pectoris occurring during an inpatient hospitalization within 180 days prior to index date"
    },
    {
      "name": "Inpatient encounter",
      "domain": "encounter",
      "intent": "hospitalization record within 180 days prior to index date"
    }
  ]
}
```
```

### 5. validation_error

```json
{
 "message": "AgentError: concepts[4].domain is 'encounter'; it must be exactly one of condition, medication, procedure, observation"
}
```

### 6. retry (attempt 1), verbatim feedback returned to the model:

```
That reply was rejected by the schema validator.

Error: AgentError: concepts[4].domain is 'encounter'; it must be exactly one of condition, medication, procedure, observation

Return the corrected JSON only, with no commentary and no code fence. Do not restate the task. Fix exactly the error above and keep everything else identical.
```

### 7. llm_request -> gemini-3.7-flash-medium  cassette `c8724f8cf3c116ae`

```
[system]
You prepare clinical trial eligibility criteria for automated checking against
structured medical records.

You are careful about what a medical record can and cannot settle. Most criteria
in a real protocol cannot be settled from a record at all, and saying so is the
correct answer rather than a failure.

[user]
Decide whether this criterion can be checked from a structured medical record,
and list the clinical concepts it needs.

Criterion (exclusion, category temporal_event):
  Myocardial infarction, stroke, transient ischaemic attack, or hospitalization for unstable angina pectoris within 180 days before screening.

A criterion is checkable ONLY if every part of it can be settled from coded
record data: demographics, coded diagnoses, coded medications, coded procedures,
or numeric laboratory and vital-sign values, with dates.

The record carries an index date: the day this prescreen is being run. Study
events that have not happened yet are anchored to it. "at screening", "at
randomisation", "at the time of signing the informed consent form", "at baseline",
"prior to randomisation" all mean "as of the index date", and a window measured
back from one of them is measured back from the index date. That substitution is
what prescreening is, so it does not make a criterion uncheckable. Say in `reason`
that you used it, so the reviewer sees the assumption.

What stays uncheckable is the participant, not the calendar: whether they will
consent, whether they are able to comply, what the investigator will judge.

It is NOT checkable if it needs any of:
  - consent, willingness, capacity, ability to comply or to attend
  - the investigator's opinion, or a judgement about suitability
  - a measurement a record does not normally hold (waist circumference,
    performance status, imaging read-outs, questionnaire scores, elastography)
  - counting prior lines of therapy, or a washout expressed in half-lives
  - a bound relative to a laboratory's own reference range, such as "2.5x the
    upper limit of normal", since the limit is not a fixed number
  - contraception, pregnancy intention, or residence and travel plans
  - enrolment in another study

Return JSON only:

{"checkable": true,
  "reason": "one sentence; if not checkable, name the specific blocker",
  "concepts": [{"name":"SGLT2 inhibitor","domain":"medication",
                 "intent":"currently prescribed drug of this class"}]}

`concepts` lists every clinical thing that must be looked up by code. Leave it
empty when the criterion uses only age or sex. Use the exact domain the record
would store it in.

[assistant]
```json
{
  "checkable": true,
  "reason": "All specified cardiovascular and cerebrovascular events and hospitalizations can be identified via coded diagnoses and inpatient encounter records within the 180 days prior to the index date, anchoring screening to the index date.",
  "concepts": [
    {
      "name": "Myocardial infarction",
      "domain": "condition",
      "intent": "diagnosis of myocardial infarction within 180 days prior to index date"
    },
    {
      "name": "Stroke",
      "domain": "condition",
      "intent": "diagnosis of ischemic or hemorrhagic stroke within 180 days prior to index date"
    },
    {
      "name": "Transient ischaemic attack",
      "domain": "condition",
      "intent": "diagnosis of transient ischemic attack within 180 days prior to index date"
    },
    {
      "name": "Unstable angina pectoris",
      "domain": "condition",
      "intent": "diagnosis of unstable angina pectoris occurring during an inpatient hospitalization within 180 days prior to index date"
    },
    {
      "name": "Inpatient encounter",
      "domain": "encounter",
      "intent": "hospitalization record within 180 days prior to index date"
    }
  ]
}
```

[user]
That reply was rejected by the schema validator.

Error: AgentError: concepts[4].domain is 'encounter'; it must be exactly one of condition, medication, procedure, observation

Return the corrected JSON only, with no commentary and no code fence. Do not restate the task. Fix exactly the error above and keep everything else identical.
```

### 8. llm_response (openai, 295 tok, 19.30871295928955s)

```
{
  "checkable": true,
  "reason": "All specified cardiovascular and cerebrovascular events and hospitalizations can be identified via coded diagnoses and inpatient encounter records within the 180 days prior to the index date, anchoring screening to the index date.",
  "concepts": [
    {
      "name": "Myocardial infarction",
      "domain": "condition",
      "intent": "diagnosis of myocardial infarction within 180 days prior to index date"
    },
    {
      "name": "Stroke",
      "domain": "condition",
      "intent": "diagnosis of ischemic or hemorrhagic stroke within 180 days prior to index date"
    },
    {
      "name": "Transient ischaemic attack",
      "domain": "condition",
      "intent": "diagnosis of transient ischemic attack within 180 days prior to index date"
    },
    {
      "name": "Unstable angina pectoris",
      "domain": "condition",
      "intent": "diagnosis of unstable angina pectoris occurring during an inpatient hospitalization within 180 days prior to index date"
    },
    {
      "name": "Inpatient encounter",
      "domain": "procedure",
      "intent": "hospitalization record within 180 days prior to index date"
    }
  ]
}
```

### 9. tool_call `ground_cache.hit`

```json
{
 "key": "condition::myocardial infarction"
}
```

### 10. tool_result `ground_cache.hit`

```json
{
 "broader_codes": [],
 "codes": [
  "22298006"
 ],
 "concept": "Myocardial infarction",
 "confidence": "high",
 "displays": [
  "Myocardial Infarction"
 ],
 "domain": "condition",
 "expanded_names": [
  "Acute myocardial infarction",
  "ST elevation myocardial infarction",
  "Non-ST elevation myocardial infarction",
  "STEMI",
  "NSTEMI",
  "Acute anterior wall myocardial infarction",
  "Acute inferior wall myocardial infarction",
  "Acute lateral wall myocardial infarction",
  "Acute posterior wall myocardial infarction",
  "Acute anteroseptal myocardial infarction",
  "Subendocardial myocardial infarction",
  "Transmural myocardial infarction",
  "Type 2 myocardial infarction",
  "Subsequent myocardial infarction",
  "Old myocardial infarction"
 ],
 "matched_names": [],
 "reason": "1 of 12 candidates selected",
 "status": "PARTIAL",
 "unmatched_names": [
  "Acute anterior wall myocardial infarction",
  "Acute anteroseptal myocardial infarction",
  "Acute inferior wall myocardial infarction",
  "Acute lateral wall myocardial infarction",
  "Acute myocardial infarction",
  "Acute posterior wall myocardial infarction",
  "NSTEMI",
  "Non-ST elevation myocardial infarction",
  "Old myocardial infarction",
  "ST elevation myocardial infarction",
  "STEMI",
  "Subendocardial myocardial infarction",
  "Subsequent myocardial infarction",
  "Transmural myocardial infarction",
  "Type 2 myocardial infarction"
 ]
}
```

### 11. tool_call `ground_cache.hit`

```json
{
 "key": "condition::stroke"
}
```

### 12. tool_result `ground_cache.hit`

```json
{
 "broader_codes": [],
 "codes": [
  "230690007"
 ],
 "concept": "Stroke",
 "confidence": "high",
 "displays": [
  "Stroke"
 ],
 "domain": "condition",
 "expanded_names": [
  "Cerebrovascular accident",
  "Ischemic stroke",
  "Cerebral infarction",
  "Acute ischemic stroke",
  "Hemorrhagic stroke",
  "Intracerebral hemorrhage",
  "Subarachnoid hemorrhage",
  "Transient ischemic attack",
  "Lacunar infarction",
  "Embolic cerebral infarction",
  "Thrombotic cerebral infarction",
  "Brainstem infarction",
  "Cerebellar infarction"
 ],
 "matched_names": [],
 "reason": "1 of 12 candidates selected",
 "status": "PARTIAL",
 "unmatched_names": [
  "Acute ischemic stroke",
  "Brainstem infarction",
  "Cerebellar infarction",
  "Cerebral infarction",
  "Cerebrovascular accident",
  "Embolic cerebral infarction",
  "Hemorrhagic stroke",
  "Intracerebral hemorrhage",
  "Ischemic stroke",
  "Lacunar infarction",
  "Subarachnoid hemorrhage",
  "Thrombotic cerebral infarction",
  "Transient ischemic attack"
 ]
}
```

### 13. instructions (prompt grounder-v3)

```
You are a clinical terminologist. You expand a clinical concept into the
specific named things a medical record would actually store for it.

Expand this clinical concept into concrete searchable names.

Concept: {concept}
Record domain: {domain}

For a drug class, list the generic ingredient names that belong to the class.
For a diagnosis, list the specific condition names, including the common
sub-types a record might code instead of the general term.
For a laboratory test, list the names and common synonyms of the measurement.

List what belongs to the concept in general clinical practice. Do NOT try to
guess what this particular site happens to record; that is looked up separately.

Return JSON only:
{{"names": ["empagliflozin", "dapagliflozin"], "note": "one short line"}}

Give between 1 and 15 names.

---

You match a clinical concept against a specific site's coded vocabulary.

You choose only from the candidates given. You never invent a code. If none of
the candidates genuinely represents the concept, you say so plainly, because a
wrong match here is worse than no match.

Concept: {concept}
Record domain: {domain}
Meaning required: {intent}

Candidate entries from this site's vocabulary:
{candidates}

Select every candidate that genuinely represents the concept. Judge by what the
entry means, not by whether the words look similar. A different measurement that
shares a word is not a match: "Respiratory rate" is not a glomerular filtration
rate, and "Chronic sinusitis" is not chronic kidney disease.

The display text beside each code is whatever the source system chose to store.
It is frequently abbreviated, locally worded, or vaguer than the code it labels,
and it is not the code's definition. Judge the concept the code denotes. Where you
recognise a code and its display disagree about how specific the concept is, the
code decides, and name the code you relied on in `why` for the ones you rejected.
This does not license guessing: a code you do not recognise is judged on its
display like any other candidate, and a display that is vaguer than the concept is
still a reason to reject a code you have no independent knowledge of.

For a drug class, select the entries whose ingredient belongs to that class.
Different strengths or formulations of the same ingredient are all matches.

A candidate can also be BROADER than the concept: it contains what the criterion
asks for and other things as well. A site that records every diabetes diagnosis
under one unspecified diabetes code has a code that is broader than "type 2
diabetes mellitus". A site whose only asthma code is "chronic lower respiratory
disease" has one that is broader than asthma.

Put those in `broader_codes`, not in `codes`. The difference decides what happens
to a patient. A code in `codes` proves the concept when it is present. A code in
`broader_codes` cannot prove it, and the criterion comes back undetermined for
that patient rather than satisfied. Its absence still counts: a patient with no
diabetes code of any kind does not have type 2 diabetes either.

Do not use `broader_codes` for a code that is merely related, or for a
complication of the concept, or for a different thing that shares a word. Only
for a code whose meaning genuinely contains the concept.

If no candidate represents the concept and none contains it, return both lists
empty. That is a normal and useful answer.

Return JSON only:
{{"codes": ["4548-4"], "broader_codes": [],
  "rejected": [{{"code": "718-7", "why": "different analyte"}}],
  "confidence": "high"}}
```

### 14. input

```json
{
 "concept": "Transient ischaemic attack",
 "domain": "condition",
 "intent": "diagnosis of transient ischemic attack within 180 days prior to index date"
}
```

### 15. llm_request -> gemini-3.7-flash-medium  cassette `f990129f717ff8d3`

```
[system]
You are a clinical terminologist. You expand a clinical concept into the
specific named things a medical record would actually store for it.

[user]
Expand this clinical concept into concrete searchable names.

Concept: Transient ischaemic attack
Record domain: condition

For a drug class, list the generic ingredient names that belong to the class.
For a diagnosis, list the specific condition names, including the common
sub-types a record might code instead of the general term.
For a laboratory test, list the names and common synonyms of the measurement.

List what belongs to the concept in general clinical practice. Do NOT try to
guess what this particular site happens to record; that is looked up separately.

Return JSON only:
{"names": ["empagliflozin", "dapagliflozin"], "note": "one short line"}

Give between 1 and 15 names.
```

### 16. llm_response (openai, 118 tok, 21.89635419845581s)

```
```json
{
  "names": [
    "Transient ischemic attack",
    "Transient ischaemic attack",
    "TIA",
    "Amaurosis fugax",
    "Transient cerebral ischemia",
    "Vertebrobasilar insufficiency",
    "Carotid artery syndrome",
    "Basilar artery syndrome",
    "Vertebral artery syndrome",
    "Hemispheric transient ischemic attack",
    "Transient retinal ischemia"
  ],
  "note": "Includes common acronyms, spelling variants, and vascular territory subtypes."
}
```
```

### 17. tool_call `terminology.search_any`

```json
{
 "domain": "condition",
 "limit": 12,
 "terms": [
  "Transient ischemic attack",
  "Transient ischaemic attack",
  "TIA",
  "Amaurosis fugax",
  "Transient cerebral ischemia",
  "Vertebrobasilar insufficiency",
  "Carotid artery syndrome",
  "Basilar artery syndrome",
  "Vertebral artery syndrome",
  "Hemispheric transient ischemic attack",
  "Transient retinal ischemia"
 ]
}
```

### 18. tool_result `terminology.search_any`

```json
[
 {
  "code": "128188000",
  "display": "Cerebral palsy (disorder)",
  "domain": "condition",
  "n_resources": 3
 },
 {
  "code": "237602007",
  "display": "Metabolic syndrome X (disorder)",
  "domain": "condition",
  "n_resources": 29
 },
 {
  "code": "95417003",
  "display": "Primary fibromyalgia syndrome",
  "domain": "condition",
  "n_resources": 2
 },
 {
  "code": "67782005",
  "display": "Acute respiratory distress syndrome (disorder)",
  "domain": "condition",
  "n_resources": 2
 }
]
```

### 19. llm_request -> gemini-3.7-flash-medium  cassette `9c1242df7fb0c9db`

```
[system]
You match a clinical concept against a specific site's coded vocabulary.

You choose only from the candidates given. You never invent a code. If none of
the candidates genuinely represents the concept, you say so plainly, because a
wrong match here is worse than no match.

[user]
Concept: Transient ischaemic attack
Record domain: condition
Meaning required: diagnosis of transient ischemic attack within 180 days prior to index date

Candidate entries from this site's vocabulary:
         128188000  Cerebral palsy (disorder)   (in 3 resources)
         237602007  Metabolic syndrome X (disorder)   (in 29 resources)
          95417003  Primary fibromyalgia syndrome   (in 2 resources)
          67782005  Acute respiratory distress syndrome (disorder)   (in 2 resources)

Select every candidate that genuinely represents the concept. Judge by what the
entry means, not by whether the words look similar. A different measurement that
shares a word is not a match: "Respiratory rate" is not a glomerular filtration
rate, and "Chronic sinusitis" is not chronic kidney disease.

The display text beside each code is whatever the source system chose to store.
It is frequently abbreviated, locally worded, or vaguer than the code it labels,
and it is not the code's definition. Judge the concept the code denotes. Where you
recognise a code and its display disagree about how specific the concept is, the
code decides, and name the code you relied on in `why` for the ones you rejected.
This does not license guessing: a code you do not recognise is judged on its
display like any other candidate, and a display that is vaguer than the concept is
still a reason to reject a code you have no independent knowledge of.

For a drug class, select the entries whose ingredient belongs to that class.
Different strengths or formulations of the same ingredient are all matches.

A candidate can also be BROADER than the concept: it contains what the criterion
asks for and other things as well. A site that records every diabetes diagnosis
under one unspecified diabetes code has a code that is broader than "type 2
diabetes mellitus". A site whose only asthma code is "chronic lower respiratory
disease" has one that is broader than asthma.

Put those in `broader_codes`, not in `codes`. The difference decides what happens
to a patient. A code in `codes` proves the concept when it is present. A code in
`broader_codes` cannot prove it, and the criterion comes back undetermined for
that patient rather than satisfied. Its absence still counts: a patient with no
diabetes code of any kind does not have type 2 diabetes either.

Do not use `broader_codes` for a code that is merely related, or for a
complication of the concept, or for a different thing that shares a word. Only
for a code whose meaning genuinely contains the concept.

If no candidate represents the concept and none contains it, return both lists
empty. That is a normal and useful answer.

Return JSON only:
{"codes": ["4548-4"], "broader_codes": [],
  "rejected": [{"code": "718-7", "why": "different analyte"}],
  "confidence": "high"}
```

### 20. llm_response (openai, 188 tok, 17.880462408065796s)

```
```json
{
  "codes": [],
  "broader_codes": [],
  "rejected": [
    {
      "code": "128188000",
      "why": "Cerebral palsy is a non-progressive developmental movement disorder, not a transient ischemic attack"
    },
    {
      "code": "237602007",
      "why": "Metabolic syndrome X is a metabolic disorder/cluster of cardiovascular risk factors, not a transient ischemic attack"
    },
    {
      "code": "95417003",
      "why": "Primary fibromyalgia syndrome is a chronic musculoskeletal pain condition, not a cerebrovascular event"
    },
    {
      "code": "67782005",
      "why": "Acute respiratory distress syndrome is a severe pulmonary inflammatory condition, not a transient ischemic attack"
    }
  ],
  "confidence": "high"
}
```
```

### 21. final

```json
{
 "broader_codes": [],
 "codes": [],
 "concept": "Transient ischaemic attack",
 "confidence": "high",
 "domain": "condition",
 "expanded_names": [
  "Transient ischemic attack",
  "Transient ischaemic attack",
  "TIA",
  "Amaurosis fugax",
  "Transient cerebral ischemia",
  "Vertebrobasilar insufficiency",
  "Carotid artery syndrome",
  "Basilar artery syndrome",
  "Vertebral artery syndrome",
  "Hemispheric transient ischemic attack",
  "Transient retinal ischemia"
 ],
 "reason": "the vocabulary returned candidates but none of them represents the concept",
 "rejected": [
  {
   "code": "128188000",
   "why": "Cerebral palsy is a non-progressive developmental movement disorder, not a transient ischemic attack"
  },
  {
   "code": "237602007",
   "why": "Metabolic syndrome X is a metabolic disorder/cluster of cardiovascular risk factors, not a transient ischemic attack"
  },
  {
   "code": "95417003",
   "why": "Primary fibromyalgia syndrome is a chronic musculoskeletal pain condition, not a cerebrovascular event"
  },
  {
   "code": "67782005",
   "why": "Acute respiratory distress syndrome is a severe pulmonary inflammatory condition, not a transient ischemic attack"
  }
 ],
 "status": "UNMAPPABLE"
}
```

### 22. instructions (prompt grounder-v3)

```
You are a clinical terminologist. You expand a clinical concept into the
specific named things a medical record would actually store for it.

Expand this clinical concept into concrete searchable names.

Concept: {concept}
Record domain: {domain}

For a drug class, list the generic ingredient names that belong to the class.
For a diagnosis, list the specific condition names, including the common
sub-types a record might code instead of the general term.
For a laboratory test, list the names and common synonyms of the measurement.

List what belongs to the concept in general clinical practice. Do NOT try to
guess what this particular site happens to record; that is looked up separately.

Return JSON only:
{{"names": ["empagliflozin", "dapagliflozin"], "note": "one short line"}}

Give between 1 and 15 names.

---

You match a clinical concept against a specific site's coded vocabulary.

You choose only from the candidates given. You never invent a code. If none of
the candidates genuinely represents the concept, you say so plainly, because a
wrong match here is worse than no match.

Concept: {concept}
Record domain: {domain}
Meaning required: {intent}

Candidate entries from this site's vocabulary:
{candidates}

Select every candidate that genuinely represents the concept. Judge by what the
entry means, not by whether the words look similar. A different measurement that
shares a word is not a match: "Respiratory rate" is not a glomerular filtration
rate, and "Chronic sinusitis" is not chronic kidney disease.

The display text beside each code is whatever the source system chose to store.
It is frequently abbreviated, locally worded, or vaguer than the code it labels,
and it is not the code's definition. Judge the concept the code denotes. Where you
recognise a code and its display disagree about how specific the concept is, the
code decides, and name the code you relied on in `why` for the ones you rejected.
This does not license guessing: a code you do not recognise is judged on its
display like any other candidate, and a display that is vaguer than the concept is
still a reason to reject a code you have no independent knowledge of.

For a drug class, select the entries whose ingredient belongs to that class.
Different strengths or formulations of the same ingredient are all matches.

A candidate can also be BROADER than the concept: it contains what the criterion
asks for and other things as well. A site that records every diabetes diagnosis
under one unspecified diabetes code has a code that is broader than "type 2
diabetes mellitus". A site whose only asthma code is "chronic lower respiratory
disease" has one that is broader than asthma.

Put those in `broader_codes`, not in `codes`. The difference decides what happens
to a patient. A code in `codes` proves the concept when it is present. A code in
`broader_codes` cannot prove it, and the criterion comes back undetermined for
that patient rather than satisfied. Its absence still counts: a patient with no
diabetes code of any kind does not have type 2 diabetes either.

Do not use `broader_codes` for a code that is merely related, or for a
complication of the concept, or for a different thing that shares a word. Only
for a code whose meaning genuinely contains the concept.

If no candidate represents the concept and none contains it, return both lists
empty. That is a normal and useful answer.

Return JSON only:
{{"codes": ["4548-4"], "broader_codes": [],
  "rejected": [{{"code": "718-7", "why": "different analyte"}}],
  "confidence": "high"}}
```

### 23. input

```json
{
 "concept": "Unstable angina pectoris",
 "domain": "condition",
 "intent": "diagnosis of unstable angina pectoris occurring during an inpatient hospitalization within 180 days prior to index date"
}
```

### 24. llm_request -> gemini-3.7-flash-medium  cassette `cbec4e6fdc406b76`

```
[system]
You are a clinical terminologist. You expand a clinical concept into the
specific named things a medical record would actually store for it.

[user]
Expand this clinical concept into concrete searchable names.

Concept: Unstable angina pectoris
Record domain: condition

For a drug class, list the generic ingredient names that belong to the class.
For a diagnosis, list the specific condition names, including the common
sub-types a record might code instead of the general term.
For a laboratory test, list the names and common synonyms of the measurement.

List what belongs to the concept in general clinical practice. Do NOT try to
guess what this particular site happens to record; that is looked up separately.

Return JSON only:
{"names": ["empagliflozin", "dapagliflozin"], "note": "one short line"}

Give between 1 and 15 names.
```

### 25. llm_response (openai, 118 tok, 18.483649969100952s)

```
```json
{
  "names": [
    "Unstable angina",
    "Unstable angina pectoris",
    "Crescendo angina",
    "Preinfarction syndrome",
    "Intermediate coronary syndrome",
    "Accelerated angina",
    "Postinfarction angina",
    "Rest angina",
    "Acute coronary insufficiency",
    "New-onset severe angina"
  ],
  "note": "Includes standard ICD/SNOMED diagnostic descriptors, presentations, and historical clinical synonyms for unstable coronary artery disease."
}
```
```

### 26. tool_call `terminology.search_any`

```json
{
 "domain": "condition",
 "limit": 12,
 "terms": [
  "Unstable angina",
  "Unstable angina pectoris",
  "Crescendo angina",
  "Preinfarction syndrome",
  "Intermediate coronary syndrome",
  "Accelerated angina",
  "Postinfarction angina",
  "Rest angina",
  "Acute coronary insufficiency",
  "New-onset severe angina"
 ]
}
```

### 27. tool_result `terminology.search_any`

```json
[
 {
  "code": "237602007",
  "display": "Metabolic syndrome X (disorder)",
  "domain": "condition",
  "n_resources": 29
 },
 {
  "code": "95417003",
  "display": "Primary fibromyalgia syndrome",
  "domain": "condition",
  "n_resources": 2
 },
 {
  "code": "67782005",
  "display": "Acute respiratory distress syndrome (disorder)",
  "domain": "condition",
  "n_resources": 2
 },
 {
  "code": "10509002",
  "display": "Acute bronchitis (disorder)",
  "domain": "condition",
  "n_resources": 266
 },
 {
  "code": "65275009",
  "display": "Acute Cholecystitis",
  "domain": "condition",
  "n_resources": 2
 },
 {
  "code": "195662009",
  "display": "Acute viral pharyngitis (disorder)",
  "domain": "condition",
  "n_resources": 347
 },
 {
  "code": "75498004",
  "display": "Acute bacterial sinusitis (disorder)",
  "domain": "condition",
  "n_resources": 38
 },
 {
  "code": "53741008",
  "display": "Coronary Heart Disease",
  "domain": "condition",
  "n_resources": 25
 },
 {
  "code": "241929008",
  "display": "Acute allergic reaction",
  "domain": "condition",
  "n_resources": 6
 },
 {
  "code": "706870000",
  "display": "Acute pulmonary embolism (disorder)",
  "domain": "condition",
  "n_resources": 4
 },
 {
  "code": "65710008",
  "display": "Acute respiratory failure (disorder)",
  "domain": "condition",
  "n_resources": 2
 },
 {
  "code": "132281000119108",
  "display": "Acute deep venous thrombosis (disorder)",
  "domain": "condition",
  "n_resources": 2
 }
]
```

### 28. llm_request -> gemini-3.7-flash-medium  cassette `3a34a7c0c5d31921`

```
[system]
You match a clinical concept against a specific site's coded vocabulary.

You choose only from the candidates given. You never invent a code. If none of
the candidates genuinely represents the concept, you say so plainly, because a
wrong match here is worse than no match.

[user]
Concept: Unstable angina pectoris
Record domain: condition
Meaning required: diagnosis of unstable angina pectoris occurring during an inpatient hospitalization within 180 days prior to index date

Candidate entries from this site's vocabulary:
         237602007  Metabolic syndrome X (disorder)   (in 29 resources)
          95417003  Primary fibromyalgia syndrome   (in 2 resources)
          67782005  Acute respiratory distress syndrome (disorder)   (in 2 resources)
          10509002  Acute bronchitis (disorder)   (in 266 resources)
          65275009  Acute Cholecystitis   (in 2 resources)
         195662009  Acute viral pharyngitis (disorder)   (in 347 resources)
          75498004  Acute bacterial sinusitis (disorder)   (in 38 resources)
          53741008  Coronary Heart Disease   (in 25 resources)
         241929008  Acute allergic reaction   (in 6 resources)
         706870000  Acute pulmonary embolism (disorder)   (in 4 resources)
          65710008  Acute respiratory failure (disorder)   (in 2 resources)
   132281000119108  Acute deep venous thrombosis (disorder)   (in 2 resources)

Select every candidate that genuinely represents the concept. Judge by what the
entry means, not by whether the words look similar. A different measurement that
shares a word is not a match: "Respiratory rate" is not a glomerular filtration
rate, and "Chronic sinusitis" is not chronic kidney disease.

The display text beside each code is whatever the source system chose to store.
It is frequently abbreviated, locally worded, or vaguer than the code it labels,
and it is not the code's definition. Judge the concept the code denotes. Where you
recognise a code and its display disagree about how specific the concept is, the
code decides, and name the code you relied on in `why` for the ones you rejected.
This does not license guessing: a code you do not recognise is judged on its
display like any other candidate, and a display that is vaguer than the concept is
still a reason to reject a code you have no independent knowledge of.

For a drug class, select the entries whose ingredient belongs to that class.
Different strengths or formulations of the same ingredient are all matches.

A candidate can also be BROADER than the concept: it contains what the criterion
asks for and other things as well. A site that records every diabetes diagnosis
under one unspecified diabetes code has a code that is broader than "type 2
diabetes mellitus". A site whose only asthma code is "chronic lower respiratory
disease" has one that is broader than asthma.

Put those in `broader_codes`, not in `codes`. The difference decides what happens
to a patient. A code in `codes` proves the concept when it is present. A code in
`broader_codes` cannot prove it, and the criterion comes back undetermined for
that patient rather than satisfied. Its absence still counts: a patient with no
diabetes code of any kind does not have type 2 diabetes either.

Do not use `broader_codes` for a code that is merely related, or for a
complication of the concept, or for a different thing that shares a word. Only
for a code whose meaning genuinely contains the concept.

If no candidate represents the concept and none contains it, return both lists
empty. That is a normal and useful answer.

Return JSON only:
{"codes": ["4548-4"], "broader_codes": [],
  "rejected": [{"code": "718-7", "why": "different analyte"}],
  "confidence": "high"}
```

### 29. llm_response (openai, 490 tok, 21.61282229423523s)

```
```json
{
  "codes": [],
  "broader_codes": [],
  "rejected": [
    {
      "code": "237602007",
      "why": "represents metabolic syndrome, a cluster of metabolic risk factors, not unstable angina pectoris"
    },
    {
      "code": "95417003",
      "why": "represents primary fibromyalgia syndrome, a chronic musculoskeletal pain disorder, not unstable angina pectoris"
    },
    {
      "code": "67782005",
      "why": "represents acute respiratory distress syndrome, a pulmonary condition, not unstable angina pectoris"
    },
    {
      "code": "10509002",
      "why": "represents acute bronchitis, a lower respiratory tract infection, not unstable angina pectoris"
    },
    {
      "code": "65275009",
      "why": "represents acute cholecystitis, gallbladder inflammation, not unstable angina pectoris"
    },
    {
      "code": "195662009",
      "why": "represents acute viral pharyngitis, an upper respiratory infection, not unstable angina pectoris"
    },
    {
      "code": "75498004",
      "why": "represents acute bacterial sinusitis, an infection of the paranasal sinuses, not unstable angina pectoris"
    },
    {
      "code": "53741008",
      "why": "represents coronary arteriosclerosis / coronary heart disease in general, an underlying structural condition rather than the acute clinical syndrome of unstable angina pectoris"
    },
    {
      "code": "241929008",
      "why": "represents an acute allergic reaction, not unstable angina pectoris"
    },
    {
      "code": "706870000",
      "why": "represents acute pulmonary embolism, a vascular pulmonary condition, not unstable angina pectoris"
    },
    {
      "code": "65710008",
      "why": "represents acute respiratory failure, not unstable angina pectoris"
    },
    {
      "code": "132281000119108",
      "why": "represents acute deep venous thrombosis, a peripheral venous disease, not unstable angina pectoris"
    }
  ],
  "confidence": "high"
}
```
```

### 30. final

```json
{
 "broader_codes": [],
 "codes": [],
 "concept": "Unstable angina pectoris",
 "confidence": "high",
 "domain": "condition",
 "expanded_names": [
  "Unstable angina",
  "Unstable angina pectoris",
  "Crescendo angina",
  "Preinfarction syndrome",
  "Intermediate coronary syndrome",
  "Accelerated angina",
  "Postinfarction angina",
  "Rest angina",
  "Acute coronary insufficiency",
  "New-onset severe angina"
 ],
 "reason": "the vocabulary returned candidates but none of them represents the concept",
 "rejected": [
  {
   "code": "237602007",
   "why": "represents metabolic syndrome, a cluster of metabolic risk factors, not unstable angina pectoris"
  },
  {
   "code": "95417003",
   "why": "represents primary fibromyalgia syndrome, a chronic musculoskeletal pain disorder, not unstable angina pectoris"
  },
  {
   "code": "67782005",
   "why": "represents acute respiratory distress syndrome, a pulmonary condition, not unstable angina pectoris"
  },
  {
   "code": "10509002",
   "why": "represents acute bronchitis, a lower respiratory tract infection, not unstable angina pectoris"
  },
  {
   "code": "65275009",
   "why": "represents acute cholecystitis, gallbladder inflammation, not unstable angina pectoris"
  },
  {
   "code": "195662009",
   "why": "represents acute viral pharyngitis, an upper respiratory infection, not unstable angina pectoris"
  },
  {
   "code": "75498004",
   "why": "represents acute bacterial sinusitis, an infection of the paranasal sinuses, not unstable angina pectoris"
  },
  {
   "code": "53741008",
   "why": "represents coronary arteriosclerosis / coronary heart disease in general, an underlying structural condition rather than the acute clinical syndrome of unstable angina pectoris"
  },
  {
   "code": "241929008",
   "why": "represents an a
```

### 31. instructions (prompt grounder-v3)

```
You are a clinical terminologist. You expand a clinical concept into the
specific named things a medical record would actually store for it.

Expand this clinical concept into concrete searchable names.

Concept: {concept}
Record domain: {domain}

For a drug class, list the generic ingredient names that belong to the class.
For a diagnosis, list the specific condition names, including the common
sub-types a record might code instead of the general term.
For a laboratory test, list the names and common synonyms of the measurement.

List what belongs to the concept in general clinical practice. Do NOT try to
guess what this particular site happens to record; that is looked up separately.

Return JSON only:
{{"names": ["empagliflozin", "dapagliflozin"], "note": "one short line"}}

Give between 1 and 15 names.

---

You match a clinical concept against a specific site's coded vocabulary.

You choose only from the candidates given. You never invent a code. If none of
the candidates genuinely represents the concept, you say so plainly, because a
wrong match here is worse than no match.

Concept: {concept}
Record domain: {domain}
Meaning required: {intent}

Candidate entries from this site's vocabulary:
{candidates}

Select every candidate that genuinely represents the concept. Judge by what the
entry means, not by whether the words look similar. A different measurement that
shares a word is not a match: "Respiratory rate" is not a glomerular filtration
rate, and "Chronic sinusitis" is not chronic kidney disease.

The display text beside each code is whatever the source system chose to store.
It is frequently abbreviated, locally worded, or vaguer than the code it labels,
and it is not the code's definition. Judge the concept the code denotes. Where you
recognise a code and its display disagree about how specific the concept is, the
code decides, and name the code you relied on in `why` for the ones you rejected.
This does not license guessing: a code you do not recognise is judged on its
display like any other candidate, and a display that is vaguer than the concept is
still a reason to reject a code you have no independent knowledge of.

For a drug class, select the entries whose ingredient belongs to that class.
Different strengths or formulations of the same ingredient are all matches.

A candidate can also be BROADER than the concept: it contains what the criterion
asks for and other things as well. A site that records every diabetes diagnosis
under one unspecified diabetes code has a code that is broader than "type 2
diabetes mellitus". A site whose only asthma code is "chronic lower respiratory
disease" has one that is broader than asthma.

Put those in `broader_codes`, not in `codes`. The difference decides what happens
to a patient. A code in `codes` proves the concept when it is present. A code in
`broader_codes` cannot prove it, and the criterion comes back undetermined for
that patient rather than satisfied. Its absence still counts: a patient with no
diabetes code of any kind does not have type 2 diabetes either.

Do not use `broader_codes` for a code that is merely related, or for a
complication of the concept, or for a different thing that shares a word. Only
for a code whose meaning genuinely contains the concept.

If no candidate represents the concept and none contains it, return both lists
empty. That is a normal and useful answer.

Return JSON only:
{{"codes": ["4548-4"], "broader_codes": [],
  "rejected": [{{"code": "718-7", "why": "different analyte"}}],
  "confidence": "high"}}
```

### 32. input

```json
{
 "concept": "Inpatient encounter",
 "domain": "procedure",
 "intent": "hospitalization record within 180 days prior to index date"
}
```

### 33. llm_request -> gemini-3.7-flash-medium  cassette `0c3dc2c79f5e55c9`

```
[system]
You are a clinical terminologist. You expand a clinical concept into the
specific named things a medical record would actually store for it.

[user]
Expand this clinical concept into concrete searchable names.

Concept: Inpatient encounter
Record domain: procedure

For a drug class, list the generic ingredient names that belong to the class.
For a diagnosis, list the specific condition names, including the common
sub-types a record might code instead of the general term.
For a laboratory test, list the names and common synonyms of the measurement.

List what belongs to the concept in general clinical practice. Do NOT try to
guess what this particular site happens to record; that is looked up separately.

Return JSON only:
{"names": ["empagliflozin", "dapagliflozin"], "note": "one short line"}

Give between 1 and 15 names.
```

### 34. llm_response (openai, 183 tok, 17.438499927520752s)

```
```json
{
  "names": [
    "Initial hospital care",
    "Subsequent hospital care",
    "Hospital discharge day management",
    "Initial inpatient consultation",
    "Follow-up inpatient consultation",
    "Initial observation care",
    "Subsequent observation care",
    "Observation or inpatient hospital care",
    "Critical care, evaluation and management",
    "Initial neonatal critical care",
    "Subsequent neonatal critical care",
    "Initial pediatric critical care",
    "Subsequent pediatric critical care",
    "Inpatient stay evaluation and management"
  ],
  "note": "CPT/SNOMED procedure and E&M concepts representing inpatient admissions, daily hospital care, consultations, and critical care management."
}
```
```

### 35. tool_call `terminology.search_any`

```json
{
 "domain": "procedure",
 "limit": 12,
 "terms": [
  "Initial hospital care",
  "Subsequent hospital care",
  "Hospital discharge day management",
  "Initial inpatient consultation",
  "Follow-up inpatient consultation",
  "Initial observation care",
  "Subsequent observation care",
  "Observation or inpatient hospital care",
  "Critical care, evaluation and management",
  "Initial neonatal critical care",
  "Subsequent neonatal critical care",
  "Initial pediatric critical care",
  "Subsequent pediatric critical care",
  "Inpatient stay evaluation and management"
 ]
}
```

### 36. tool_result `terminology.search_any`

```json
[
 {
  "code": "386394001",
  "display": "Pregnancy termination care",
  "domain": "procedure",
  "n_resources": 43
 },
 {
  "code": "408919008",
  "display": "Psychosocial care (regime/therapy)",
  "domain": "procedure",
  "n_resources": 7
 },
 {
  "code": "305340004",
  "display": "Admission to long stay hospital",
  "domain": "procedure",
  "n_resources": 2
 },
 {
  "code": "710824005",
  "display": "Assessment of health and social care needs (procedure)",
  "domain": "procedure",
  "n_resources": 3840
 },
 {
  "code": "274804006",
  "display": "Evaluation of uterine fundal height",
  "domain": "procedure",
  "n_resources": 1316
 },
 {
  "code": "26763009",
  "display": "Controlled ventilation procedure and therapy, initiation and management (procedure)",
  "domain": "procedure",
  "n_resources": 10
 }
]
```

### 37. llm_request -> gemini-3.7-flash-medium  cassette `1b42ec449b192429`

```
[system]
You match a clinical concept against a specific site's coded vocabulary.

You choose only from the candidates given. You never invent a code. If none of
the candidates genuinely represents the concept, you say so plainly, because a
wrong match here is worse than no match.

[user]
Concept: Inpatient encounter
Record domain: procedure
Meaning required: hospitalization record within 180 days prior to index date

Candidate entries from this site's vocabulary:
         386394001  Pregnancy termination care   (in 43 resources)
         408919008  Psychosocial care (regime/therapy)   (in 7 resources)
         305340004  Admission to long stay hospital   (in 2 resources)
         710824005  Assessment of health and social care needs (procedure)   (in 3840 resources)
         274804006  Evaluation of uterine fundal height   (in 1316 resources)
          26763009  Controlled ventilation procedure and therapy, initiation and managemen   (in 10 resources)

Select every candidate that genuinely represents the concept. Judge by what the
entry means, not by whether the words look similar. A different measurement that
shares a word is not a match: "Respiratory rate" is not a glomerular filtration
rate, and "Chronic sinusitis" is not chronic kidney disease.

The display text beside each code is whatever the source system chose to store.
It is frequently abbreviated, locally worded, or vaguer than the code it labels,
and it is not the code's definition. Judge the concept the code denotes. Where you
recognise a code and its display disagree about how specific the concept is, the
code decides, and name the code you relied on in `why` for the ones you rejected.
This does not license guessing: a code you do not recognise is judged on its
display like any other candidate, and a display that is vaguer than the concept is
still a reason to reject a code you have no independent knowledge of.

For a drug class, select the entries whose ingredient belongs to that class.
Different strengths or formulations of the same ingredient are all matches.

A candidate can also be BROADER than the concept: it contains what the criterion
asks for and other things as well. A site that records every diabetes diagnosis
under one unspecified diabetes code has a code that is broader than "type 2
diabetes mellitus". A site whose only asthma code is "chronic lower respiratory
disease" has one that is broader than asthma.

Put those in `broader_codes`, not in `codes`. The difference decides what happens
to a patient. A code in `codes` proves the concept when it is present. A code in
`broader_codes` cannot prove it, and the criterion comes back undetermined for
that patient rather than satisfied. Its absence still counts: a patient with no
diabetes code of any kind does not have type 2 diabetes either.

Do not use `broader_codes` for a code that is merely related, or for a
complication of the concept, or for a different thing that shares a word. Only
for a code whose meaning genuinely contains the concept.

If no candidate represents the concept and none contains it, return both lists
empty. That is a normal and useful answer.

Return JSON only:
{"codes": ["4548-4"], "broader_codes": [],
  "rejected": [{"code": "718-7", "why": "different analyte"}],
  "confidence": "high"}
```

### 38. llm_response (openai, 227 tok, 19.098268508911133s)

```
```json
{
  "codes": ["305340004"],
  "broader_codes": [],
  "rejected": [
    {
      "code": "386394001",
      "why": "pregnancy termination care is a specific clinical procedure, not an inpatient encounter or hospitalization record"
    },
    {
      "code": "408919008",
      "why": "psychosocial care regime/therapy, not an inpatient encounter"
    },
    {
      "code": "710824005",
      "why": "assessment of health and social care needs is an assessment procedure, not a hospital admission or inpatient encounter"
    },
    {
      "code": "274804006",
      "why": "evaluation of uterine fundal height is an obstetric examination procedure, not an inpatient encounter"
    },
    {
      "code": "26763009",
      "why": "controlled ventilation initiation and management is a clinical intervention/therapy, not an encounter or hospitalization record"
    }
  ],
  "confidence": "high"
}
```
```

### 39. final

```json
{
 "broader_codes": [],
 "codes": [
  "305340004"
 ],
 "concept": "Inpatient encounter",
 "confidence": "high",
 "displays": [
  "Admission to long stay hospital"
 ],
 "domain": "procedure",
 "expanded_names": [
  "Initial hospital care",
  "Subsequent hospital care",
  "Hospital discharge day management",
  "Initial inpatient consultation",
  "Follow-up inpatient consultation",
  "Initial observation care",
  "Subsequent observation care",
  "Observation or inpatient hospital care",
  "Critical care, evaluation and management",
  "Initial neonatal critical care",
  "Subsequent neonatal critical care",
  "Initial pediatric critical care",
  "Subsequent pediatric critical care",
  "Inpatient stay evaluation and management"
 ],
 "matched_names": [],
 "reason": "1 of 6 candidates selected",
 "status": "PARTIAL",
 "unmatched_names": [
  "Critical care, evaluation and management",
  "Follow-up inpatient consultation",
  "Hospital discharge day management",
  "Initial hospital care",
  "Initial inpatient consultation",
  "Initial neonatal critical care",
  "Initial observation care",
  "Initial pediatric critical care",
  "Inpatient stay evaluation and management",
  "Observation or inpatient hospital care",
  "Subsequent hospital care",
  "Subsequent neonatal critical care",
  "Subsequent observation care",
  "Subsequent pediatric critical care"
 ]
}
```

### 40. final

```json
{
 "compilable": false,
 "reason_not_compilable": "cannot be represented in this site's vocabulary. Transient ischaemic attack: the vocabulary returned candidates but none of them represents the concept; Unstable angina pectoris: the vocabulary returned candidates but none of them represents the concept"
}
```

