...
```json
{
 ...
 "criterion_id": "NCT06983054-INC-08-CF",
 "kind": "inclusion",
 "source_text": "Sodium intake at baseline < 274 mmol/day"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `6b8b3811931b3dfb`
...
Criterion (inclusion, category lifestyle_or_social):
  Sodium intake at baseline < 274 mmol/day
...
### 4. llm_response (openai, 49 tok, 125.93322134017944s)

```
```json
{
  "checkable": false,
  "reason": "Dietary sodium intake is a lifestyle and nutritional measurement that is not routinely captured in structured medical records.",
  "concepts": []
}
```
```
...
{
 "compilable": false,
 "reason_not_compilable": "Dietary sodium intake is a lifestyle and nutritional measurement that is not routinely captured in structured medical records."
}
```
