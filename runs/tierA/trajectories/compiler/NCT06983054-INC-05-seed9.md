...
### 3. llm_request -> gemini-3.7-flash-medium  cassette `1b75333524f6ef7d`
...
### 4. llm_response (openai, 139 tok, 36.13875937461853s)

```
```json
{
  "checkable": false,
  "reason": "Menstrual history (no menses for >1 year) is not reliably captured in structured coded data, and the criterion involves prospective clinical judgment and testing in case of doubt.",
  "concepts": [
    {
      "name": "Postmenopausal state",
      "domain": "condition",
      "intent": "diagnosis or documented record of postmenopausal status"
    },
    {
      "name": "Follicle-stimulating hormone",
      "domain": "measurement",
      "intent": "serum FSH measurement with value >31 U/L"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "Menstrual history (no menses for >1 year) is not reliably captured in structured coded data, and the criterion involves prospective clinical judgment and testing in case of doubt."
}
```
