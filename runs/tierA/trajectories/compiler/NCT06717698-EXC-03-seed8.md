...
### 3. llm_request -> gemini-3.7-flash-medium  cassette `1e2dbc27e804dd48`
...
### 4. llm_response (openai, 110 tok, 29.253865480422974s)

```
```json
{
  "checkable": true,
  "reason": "Prescription and dispensing records for GLP-1 receptor agonists can be queried within a 90-day window anchored to the index date as the screening date.",
  "concepts": [
    {
      "name": "GLP-1 receptor agonist",
      "domain": "medication",
      "intent": "medication with GLP-1 receptor agonist activity prescribed or administered within 90 days prior to the index date"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "cannot be represented in this site's vocabulary. GLP-1 receptor agonist: no entry in this site's medication vocabulary matches any of semaglutide, liraglutide, dulaglutide, exenatide, lixisenatide, albiglutide and others"
}
```
