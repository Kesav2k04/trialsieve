...
```json
{
 "category": "investigator_judgement",
 "criterion_id": "NCT06989723-EXC-05",
 "kind": "exclusion",
 "source_text": "No changes in anti-diabetic or metabolic medications within the past 3 months, unless the changes are deemed by the investigator not to affect study outcomes."
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `cbec54cf6078c179`
...
Criterion (exclusion, category investigator_judgement):
...
### 4. llm_response (openai, 147 tok, 45.94830369949341s)

```
```json
{
  "checkable": false,
  "reason": "The criterion requires investigator judgement to determine whether medication changes affect study outcomes, which cannot be settled from structured medical records.",
  "concepts": [
    {
      "name": "anti-diabetic medication",
      "domain": "medication",
      "intent": "prescription or change in anti-diabetic medication within the past 3 months"
    },
    {
      "name": "metabolic medication",
      "domain": "medication",
      "intent": "prescription or change in metabolic medication within the past 3 months"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "The criterion requires investigator judgement to determine whether medication changes affect study outcomes, which cannot be settled from structured medical records."
}
```
