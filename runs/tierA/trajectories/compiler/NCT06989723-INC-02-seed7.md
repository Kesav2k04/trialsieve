...
```json
{
 "message": "AssertionError: criterion.expr.args[0].query: query needs a non-empty list of string codes"
}
```

### 20. retry (attempt 1), verbatim feedback returned to the model:

```
That reply was rejected by the schema validator.

Error: AssertionError: criterion.expr.args[0].query: query needs a non-empty list of string codes
...
### 21. llm_request -> gemini-3.7-flash-medium  cassette `ce6d3a5213c1838e`
...
### 22. llm_response (cassette, 125 tok, 178.907s)
...
```
{
  ...
  "unit_note": "HbA1c is stated as a percentage (%).",
  "absence_note": "No condition query with absent_means is used because the criterion is defined by the HbA1c observation and no specific condition codes were mapped.",
  "confidence": "high"
}
```
