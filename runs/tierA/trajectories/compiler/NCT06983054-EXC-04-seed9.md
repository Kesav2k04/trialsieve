...
```json
{
 ...
 "criterion_id": "NCT06983054-EXC-04",
 "kind": "exclusion",
 "source_text": "History of diabetic ketoacidosis (DKA) requiring medical intervention within 1 month prior to the Screening visit."
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `f182210e9104134b`
...
### 4. llm_response (openai, 115 tok, 26.724936723709106s)

```
```json
{
  "checkable": true,
  "reason": "Anchoring the screening visit to the index date, a history of diabetic ketoacidosis within the 1-month lookback window can be identified from coded diagnosis and encounter records.",
  "concepts": [
    {
      "name": "diabetic ketoacidosis",
      "domain": "condition",
      "intent": "coded diagnosis or acute encounter for diabetic ketoacidosis occurring within 1 month prior to the index date"
    }
  ]
}
```
...
### 7. llm_request -> gemini-3.7-flash-medium  cassette `4007d31e49392228`
...
### 8. llm_response (openai, 191 tok, 26.050787448883057s)
...
```json
{
  "names": [
    ...
    "Drug-induced diabetes mellitus with ketoacidosis"
  ],
  "note": "Includes specific diabetes types (T1D/T2D), severity subtypes (with/without coma), and variant presentations (euglycemic, drug-induced)."
}
```
```
...
### 11. llm_request -> gemini-3.7-flash-medium  cassette `47a189d7fa3a673d`
...
Concept: diabetic ketoacidosis
Record domain: condition
Meaning required: coded diagnosis or acute encounter for diabetic ketoacidosis occurring within 1 month prior to the index date
...
### 12. llm_response (openai, 402 tok, 34.66680097579956s)
...
```json
{
  ...
  "rejected": [
    {
      "code": "368581000119106",
      "why": "neuropathy due to type 2 diabetes mellitus, not diabetic ketoacidosis"
    },
    {
      "code": "90781000119102",
      "why": "microalbuminuria due to type 2 diabetes mellitus, not diabetic ketoacidosis"
    },
    {
      "code": "1551000119108",
      "why": "nonproliferative diabetic retinopathy, not diabetic ketoacidosis"
    },
    {
      "code": "97331000119101",
      "why": "macular edema and retinopathy due to type 2 diabetes mellitus, not diabetic ketoacidosis"
    },
    {
      "code": "422034002",
      "why": "diabetic retinopathy associated with type 2 diabetes mellitus, not diabetic ketoacidosis"
    },
    {
      "code": "1501000119109",
      "why": "proliferative diabetic retinopathy due to type 2 diabetes mellitus, not diabetic ketoacidosis"
    },
    {
      "code": "127013003",
      "why": "diabetic renal disease, not diabetic ketoacidosis"
    },
    {
      "code": "15777000",
      "why": "prediabetes, not diabetic ketoacidosis"
    },
    {
      "code": "44054006",
      "why": "general diabetes mellitus disorder; does not specify or represent the acute complication of diabetic ketoacidosis"
    },
    {
      "code": "55680006",
      "why": "drug overdose, unrelated condition"
    },
    {
      "code": "94260004",
      "why": "secondary malignant neoplasm of colon, unrelated condition"
    },
    {
      "code": "431855005",
      "why": "chronic kidney disease stage 1, unrelated condition"
    }
  ],
  "confidence": "high"
}
...
{
 "compilable": false,
 "reason_not_compilable": "cannot be represented in this site's vocabulary. diabetic ketoacidosis: the vocabulary returned candidates but none of them represents the concept"
}
```
