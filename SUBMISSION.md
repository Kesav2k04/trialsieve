...
- Treat it as an exact match and a criterion about type 2 diabetes silently
  answers MEETS from a code that cannot establish it. That produced 358 of 424
  wrong exclusions (entry 25).
...
- Accept it as broader-only but keep closed-world absence, and the criterion
  commits FALSE on every patient whose chart never mentions diabetes at all. That
  produced 358 wrong FAILS on one criterion and 246 on another (entry 30).
