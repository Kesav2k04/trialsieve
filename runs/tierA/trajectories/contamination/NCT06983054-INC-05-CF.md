...
```json
{
 ...
 "criterion_id": "NCT06983054-INC-05-CF",
 "kind": "inclusion",
 "source_text": "Both sexes (females must be post-menopausal; no menses >1 year; in case of doubt, Follicle-Stimulating Hormone (FSH) will be determined with cut-off defined as >42.5 U/L)"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `2ff64e82fe786cb1`
...
Criterion (inclusion, category reproductive):
  Both sexes (females must be post-menopausal; no menses >1 year; in case of doubt, Follicle-Stimulating Hormone (FSH) will be determined with cut-off defined as >42.5 U/L)
...
### 4. llm_response (openai, 130 tok, 253.85123229026794s)

```
```json
{
  "checkable": false,
  "reason": "Menstrual history (no menses >1 year) is not available in structured record data, and the contingency ('in case of doubt') requires prospective investigator evaluation.",
  "concepts": [
    {
      "name": "Postmenopausal status",
      "domain": "condition",
      "intent": "diagnosis indicating postmenopausal state"
    },
    {
      "name": "Follicle-stimulating hormone",
      "domain": "lab",
      "intent": "serum FSH level with a value >42.5 U/L"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "Menstrual history (no menses >1 year) is not available in structured record data, and the contingency ('in case of doubt') requires prospective investigator evaluation."
}
```
