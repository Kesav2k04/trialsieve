...
### 3. llm_request -> gemini-3.7-flash-medium  cassette `e55f917907f36434`
...
### 4. llm_response (cassette, 96 tok, 49.662s)

```
```json
{
  "checkable": true,
  "reason": "Adult age is verifiable from demographic birth date and previously diagnosed T2DM is identifiable through coded diagnosis history prior to the index date.",
  "concepts": [
    {
      "name": "Type 2 diabetes mellitus",
      "domain": "condition",
      "intent": "prior or existing diagnosis of type 2 diabetes mellitus"
    }
  ]
}
```
...
### 7. llm_request -> gemini-3.7-flash-medium  cassette `abaa6a839fe9af5a`
...
### 8. llm_response (cassette, 202 tok, 58.604s)
...
```json
{
  ...
  "note": "Includes standard SNOMED CT and ICD-10-CM concept expressions and common manifestation-specific subtypes."
}
```
```
...
### 11. llm_request -> gemini-3.7-flash-medium  cassette `0d8be085a931073e`
...
Concept: Type 2 diabetes mellitus
Record domain: condition
Meaning required: prior or existing diagnosis of type 2 diabetes mellitus
...
### 12. llm_response (cassette, 241 tok, 69.869s)
...
```json
{
  ...
  "rejected": [
    {
      "code": "431856006",
      "why": "chronic kidney disease stage 2, not diabetes mellitus"
    },
    {
      "code": "431855005",
      "why": "chronic kidney disease stage 1, not diabetes mellitus"
    },
    {
      "code": "15777000",
      "why": "prediabetes is an impaired glucose regulation state, not an established diagnosis of type 2 diabetes mellitus"
    },
    {
      "code": "80394007",
      "why": "hyperglycemia is a laboratory finding/sign of elevated blood glucose, not a diagnosis of diabetes mellitus"
    },
    {
      "code": "127013003",
      "why": "diabetic renal disease is a complication of unspecified diabetes mellitus that does not specify type 2 diabetes mellitus"
    }
  ],
  "confidence": "high"
}
...
```json
{
 ...
 "unmatched_names": [
  ...
  "Type 2 diabetes mellitus with diabetic chronic kidney disease",
  "Type 2 diabetes mellitus with diabetic neuropathy",
  "Type 2 diabetes mellitus with diabetic periphe
```

### 14. llm_request -> gemini-3.7-flash-medium  cassette `6b5ac1d640075172`
...
### 15. llm_response (cassette, 276 tok, 49.744s)
...
```json
{
  ...
  "unit_note": "Age is expressed in years for the adult requirement.",
  "absence_note": "absent_means is set to 'unknown' because a prior diagnosis of Type 2 diabetes mellitus may have occurred at an outside clinic or institution, so absence of a record locally does not definitively rule out prior diagnosis.",
  "confidence": "medium"
}
```
```
