...
{
 "category": "diagnosis",
 "criterion_id": "NCT06983054-INC-01-CF",
 "kind": "inclusion",
 "source_text": "Adults with previously diagnosed T2.7DM according to American Diabetes Association (ADA) criteria"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `82eece11df82a84e`
...
Criterion (inclusion, category diagnosis):
  Adults with previously diagnosed T2.7DM according to American Diabetes Association (ADA) criteria
...
### 4. llm_response (cassette, 68 tok, 58.115s)

```
```json
{
  "checkable": false,
  "reason": "T2.7DM is not a standardized coded condition in electronic health records, and confirmation that a diagnosis met specific American Diabetes Association (ADA) criteria is not recorded in structured data.",
  "concepts": []
}
```
```
...
{
 "compilable": false,
 "reason_not_compilable": "T2.7DM is not a standardized coded condition in electronic health records, and confirmation that a diagnosis met specific American Diabetes Association (ADA) criteria is not recorded in structured data."
}
```
