...
CLAIMS: list[tuple] = [
    ...
    ("lead table, the registered primary outcome for the baseline",
     ("README.md",),
     r"primary\s+outcome\s+registered\s+before\s+the\s+run\*\*\s+\|\s+\*\*(\w+)\*\*",
     lambda r: {r["groups"]["b2_10p"]["panel_scores"]["B2"]["primary_outcome"]}),
    ("lead table, the registered primary outcome for TrialSieve",
     ("README.md",),
     r"primary\s+outcome\s+registered\s+before\s+the\s+run\*\*\s+\|\s+"
     r"\*\*\w+\*\*\s+\|\s+\*\*(\w+)\*\*",
     lambda r: {r["groups"]["b2_10p"]["panel_scores"]["TS"]["primary_outcome"]}),
    ("lead table, B2 wrong MEETS",
     ("README.md",),
     r"enrols\s+someone\s+who\s+should\s+not\s+be\s+\|\s+(\d+)\s+\|",
