...
Generated by `python scripts/scorecard.py`. Every number is read from `results/results.json`, which `python run.py reproduce` regenerates offline from recorded model calls.

The rows below are scored on **the same 400 cells**: 30 patients against the criteria that compiled, judged by both arms. B2 is the only baseline that spends a model call per cell, so it was recorded over a seeded sample rather than the full 15,400-cell panel, and every arm it is compared against is re-scored on that sample rather than quoted from the larger run.
...
| Patients wrongly ruled out, of 30 | 10 | **2** | 8 fewer |
...
| **Human time per task**, as judgements a person is left holding *(one trial, 385 patients, not the sample above)* | 1,155 cell judgements, the manual process today | 190 screens carrying **2 distinct questions** | 2 question set(s) instead of 1,155 separate readings |
