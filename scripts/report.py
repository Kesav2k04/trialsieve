...
def main() -> int:
    ...
    md.append("**Who wrote gold.** Checker A is hand-authored by the author of this "
              ...
              "the IR, of A, or of any system output. This paragraph used to add "
              "that B's labels were committed before any commit carrying system "
              "output, which was true when it was written and stopped being true "
              "when B and the scored compile ended up running concurrently on one "
              "machine: commit order now records which process finished first and "
              "nothing else. Amendment A1 in `docs/EVAL_PROTOCOL.md` withdrew it, "
              "and `python scripts/verify.py blind` is the replacement, because "
              "blindness is a fact about what was in the prompt rather than about "
              "when a file was written. What the second labelling buys is a "
