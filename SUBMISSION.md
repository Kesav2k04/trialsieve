...
| A qualified human reviewer in the loop | The sign-off gate, with the reviewer's role recorded in the signature. Stated plainly: the reviewer here is the author and is not a clinician, and that is what the `reviewer_role` field says. A deployment puts a clinician in that slot. |
