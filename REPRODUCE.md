...
| Disk | **85 MB of tracked files, and a fresh clone measured 247 MB** including history. Every figure in this row is decimal MB. That command prints bytes, so divide by a million to land on these numbers; dividing by 1024 squared instead reads 81 rather than 85, which looks like the row is wrong again and is not. The tracked bulk is `runs/` at 43 MB, the recorded cassettes and trajectories that make the replay possible, then `docs/` at 19 MB, almost all of it the rendered walkthrough, then `film/` at 14 MB, almost all of it the narration the walkthrough is cut to, then the vendored panel at 7 MB. The gap between 85 and 247 is history, and most of it is the nine versions of the video this history carries: a 19 MB binary does not delta-compress, and the film was rebuilt every time a figure on one of its cards moved. Measure it yourself with `git ls-tree -r -l HEAD` and `git count-objects -vH`. This row has been wrong five times, first at 12 MB naming the panel as the bulk, then at 65 MB before the video was committed, then at 61 MB before the film was, then at 185 MB and again at 231 MB after further rebuilds, so it is worth measuring rather than reading. |
...
8. **The full suite runs**, all 346 tests, now that every artifact they read
