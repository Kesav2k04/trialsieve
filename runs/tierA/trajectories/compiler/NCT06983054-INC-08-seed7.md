...
```json
{
 ...
 "criterion_id": "NCT06983054-INC-08",
 "kind": "inclusion",
 "source_text": "Sodium intake at baseline < 200 mmol/day"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `37661dfbdfe62e2c`
...
Criterion (inclusion, category lifestyle_or_social):
...
### 4. llm_response (openai, 49 tok, 129.60027933120728s)

```
```json
{
  "checkable": false,
  "reason": "Dietary sodium intake is a lifestyle and nutritional assessment that is not routinely captured in structured medical records.",
  "concepts": []
}
```
```
...
{
 "compilable": false,
 "reason_not_compilable": "Dietary sodium intake is a lifestyle and nutritional assessment that is not routinely captured in structured medical records."
}
```
