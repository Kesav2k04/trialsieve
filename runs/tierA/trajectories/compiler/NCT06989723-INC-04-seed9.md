...
{
 "category": "procedure",
 "criterion_id": "NCT06989723-INC-04",
 "kind": "inclusion",
 "source_text": "Evidence of hepatic steatosis within the past 3 months, confirmed by Fibroscan with a controlled attenuation parameter (CAP) >= 268 dB/m"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `217dfb8a25165432`
...
Criterion (inclusion, category procedure):
...
### 4. llm_response (openai, 150 tok, 17.974820852279663s)

```
```json
{
  "checkable": false,
  "reason": "FibroScan controlled attenuation parameter (CAP) is an elastography and imaging read-out measurement that structured medical records do not normally hold as coded numeric values.",
  "concepts": [
    {
      "name": "hepatic steatosis",
      "domain": "condition",
      "intent": "evidence or diagnosis of hepatic steatosis within the past 3 months"
    },
    {
      "name": "FibroScan controlled attenuation parameter",
      "domain": "measurement",
      "intent": "CAP score >= 268 dB/m within the past 3 months from index date"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "FibroScan controlled attenuation parameter (CAP) is an elastography and imaging read-out measurement that structured medical records do not normally hold as coded numeric values."
}
```
