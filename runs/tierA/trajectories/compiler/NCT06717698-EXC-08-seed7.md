...
### 3. llm_request -> gemini-3.7-flash-medium  cassette `8b6520d75a93d1ba`
...
### 4. llm_response (openai, 61 tok, 46.97380805015564s)

```
```json
{
  "checkable": false,
  "reason": "Not checkable because assessing whether a participant intends to become pregnant requires evaluating pregnancy intention, which is not recorded in structured medical records.",
  "concepts": []
}
```
```
...
{
 "compilable": false,
 "reason_not_compilable": "Not checkable because assessing whether a participant intends to become pregnant requires evaluating pregnancy intention, which is not recorded in structured medical records."
}
```
