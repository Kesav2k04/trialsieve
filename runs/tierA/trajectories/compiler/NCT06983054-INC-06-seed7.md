...
```json
{
 ...
 "criterion_id": "NCT06983054-INC-06",
 "kind": "inclusion",
 "source_text": "Ability to provide signed and dated, written informed consent prior to any study procedures"
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `2810d1558863c30d`
...
Criterion (inclusion, category consent_or_capacity):
...
### 4. llm_response (openai, 62 tok, 21.677238941192627s)

```
```json
{
  "checkable": false,
  "reason": "Evaluating a participant's ability or willingness to provide informed consent requires assessing consent and capacity, which cannot be determined from structured medical records.",
  "concepts": []
}
```
```
...
{
 "compilable": false,
 "reason_not_compilable": "Evaluating a participant's ability or willingness to provide informed consent requires assessing consent and capacity, which cannot be determined from structured medical records."
}
```
