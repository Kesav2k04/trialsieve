...
| Solution video, 5 minutes or less | submitted as a link on the entry form, 4:57 (297.0s of a 300s limit). It is a deliverable rather than part of the solution, so it is not a file here and nothing in this repository needs it to run. |
...
| Measured Improvement | 15 | [docs/SCORECARD.md](docs/SCORECARD.md) for the baseline comparison, [docs/IMPROVEMENT_CHANGELOG.md](docs/IMPROVEMENT_CHANGELOG.md) for 79 entries each tied to the command that shows it. |
...
Those are counts of **tracked** cassettes under `runs/*/cassettes/`, so a judge can recount them in a clone rather than take the table's word for it. The scope matters and this sentence used not to state it: counting every tracked cassette directory instead gives 1,430, because `cassettes_counterfactual/` holds 33 more and `cassettes_critic_probe/` 43, both of them side experiments scored separately and neither part of the run these numbers describe. A reader who recounted the wider set would have found a table that looked wrong and was not. `python -m pytest tests/test_recorded_call_counts.py -q` is the recount, over the same scope, and it fails if this table drifts from the files, which it had: every call behind every number in this submission ships with it, including the grounding probes the hot take rests on.
