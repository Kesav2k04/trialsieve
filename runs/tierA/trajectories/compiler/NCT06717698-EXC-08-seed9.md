...
```json
{
 ...
 "criterion_id": "NCT06717698-EXC-08",
 "kind": "exclusion",
 "source_text": "Female who is pregnant, breast-feeding or intends to become pregnant."
}
```

### 3. llm_request -> gemini-3.7-flash-medium  cassette `70a9b8295926f3d7`
...
Criterion (exclusion, category reproductive):
...
### 4. llm_response (openai, 120 tok, 18.338616847991943s)

```
```json
{
  "checkable": false,
  "reason": "The criterion is not checkable because a patient's intention to become pregnant cannot be determined from structured medical records.",
  "concepts": [
    {
      "name": "Pregnancy",
      "domain": "condition",
      "intent": "current pregnancy diagnosis or positive test result"
    },
    {
      "name": "Breastfeeding",
      "domain": "observation",
      "intent": "active breast-feeding or lactation status"
    }
  ]
}
```
...
{
 "compilable": false,
 "reason_not_compilable": "The criterion is not checkable because a patient's intention to become pregnant cannot be determined from structured medical records."
}
```
